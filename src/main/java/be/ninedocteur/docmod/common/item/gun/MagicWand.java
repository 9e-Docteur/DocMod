...
public class MagicWand extends MagicLaserGunItem {
    public MagicWand(Item.Properties pProperties) {
        super(pProperties);
    }
    ...
    public void releaseUsing(ItemStack pStack, Level pLevel, LivingEntity pEntityLiving, int pTimeLeft) {
        if (pEntityLiving instanceof Player) {
            ...
            ItemStack itemstack = GunUtils.getMagicLaserProjectile(pStack, player);
            ...
            checkStrikeEnchant(player, pEntityLiving);

            if (!itemstack.isEmpty() || flag) {
                if (itemstack.isEmpty()) {
                    itemstack = new ItemStack(DMItems.MAGIC_LASER.get());
                }

                float f = getPowerForTime(i);
                if (!((double)f < 0.1D)) {
                    boolean flag1 = player.getAbilities().instabuild || itemstack.getItem() instanceof MagicLaserItem && ((MagicLaserItem)itemstack.getItem()).isInfinite(itemstack, pStack, player);
                    if (!pLevel.isClientSide) {
                        MagicLaserItem arrowitem = (MagicLaserItem)(itemstack.getItem() instanceof MagicLaserItem ? itemstack.getItem() : DMItems.MAGIC_LASER.get());
                        AbstractWandLaser abstractarrow = arrowitem.createArrow(pLevel, itemstack, player);
                        abstractarrow = this.customArrow(abstractarrow);
                        abstractarrow.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, f * 3F, 1.0F);
                        if (f == 1.0F) {
                        ...
                        if (flag1 || player.getAbilities().instabuild && (itemstack.is(Items.SPECTRAL_ARROW) || itemstack.is(Items.TIPPED_ARROW))) {
                            abstractarrow.pickup = AbstractWandLaser.Pickup.CREATIVE_ONLY;
                        }

                        pLevel.addFreshEntity(abstractarrow);
                    }

                    pLevel.playSound((Player)null, player.getX(), player.getY(), player.getZ(), DMSound.MAGIC_WAND.get(), SoundSource.PLAYERS, 1.0F, 1.0F / (pLevel.getRandom().nextFloat() * 0.4F + 1.2F) + f * 0.5F);
    ...
    public AbstractWandLaser customArrow(AbstractWandLaser arrow) {
        return arrow;
    }
    ...
    private static void checkStrikeEnchant(Player pPlayer, LivingEntity entity){
        ...
        Map<Enchantment, Integer> enchantments = EnchantmentHelper.getEnchantments(itemstack);
        int level = EnchantmentHelper.getItemEnchantmentLevel(DMEnchantements.STRIKE.get(), itemstack);
        if(level < 1){
