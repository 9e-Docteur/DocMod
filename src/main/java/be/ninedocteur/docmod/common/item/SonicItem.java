...
public class SonicItem extends SonicInteractionItem {
...
/*
    ...
    public void inventoryTick(ItemStack pStack, Level pLevel, Entity pEntity, int pSlotId, boolean pIsSelected) {
        if(pStack.getDisplayName().equals(new TextComponent("13")) && pEntity instanceof Player pPlayer) {
            pPlayer.getInventory().clearOrCountMatchingItems(p -> pStack.getItem() == p.getItem(), 1, pPlayer.inventoryMenu.getCraftSlots());
            pPlayer.getInventory().add(new ItemStack(DMItems.THEER_SONIC_SCREWDRIVER.get()));
        }

        if(pStack.getItem().getName(new ItemStack(this)).equals(new TextComponent("13"))&& pEntity instanceof Player pPlayer){
            pPlayer.getInventory().clearOrCountMatchingItems(p -> pStack.getItem() == p.getItem(), 1, pPlayer.inventoryMenu.getCraftSlots());
            pPlayer.getInventory().add(new ItemStack(DMItems.THEER_SONIC_SCREWDRIVER.get()));
        }
 ...
 */
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        pTooltipComponents.add(Component.literal(ChatFormatting.YELLOW  + "Made by: " + ChatFormatting.WHITE + "Robainks"));
