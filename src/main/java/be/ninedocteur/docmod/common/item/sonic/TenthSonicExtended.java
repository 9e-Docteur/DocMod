...
public class TenthSonicExtended extends SonicInteractionItem {
    public TenthSonicExtended(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResult useOn(UseOnContext pContext) {
        ...
        if(KeyUtils.hasShiftDown()){
            pPlayer.getInventory().clearOrCountMatchingItems(p -> new ItemStack(DMItems.TEN_SONIC_SCREWDRIVER_EXTENDED.get()).getItem() == p.getItem(), 1, pPlayer.inventoryMenu.getCraftSlots());
            pPlayer.getInventory().add(new ItemStack(DMItems.TEN_SONIC_SCREWDRIVER.get()));
        }
