...
public class DalekGun extends DalekLaserGunItem {
    public DalekGun(Properties pProperties) {
        super(pProperties);
    }
        ...
        if (pEntityLiving instanceof Player) {
            ...
            if (!itemstack.isEmpty() || flag) {
                if (itemstack.isEmpty()) {
                    itemstack = new ItemStack(DMItems.DALEK_LASER.get());
                }

                float f = getPowerForTime(i);
                if (!((double)f < 0.1D)) {
                    boolean flag1 = player.getAbilities().instabuild || itemstack.getItem() instanceof DalekLaserItem && ((DalekLaserItem)itemstack.getItem()).isInfinite(itemstack, pStack, player);
                    if (!pLevel.isClientSide) {
                        DalekLaserItem arrowitem = (DalekLaserItem)(itemstack.getItem() instanceof DalekLaserItem ? itemstack.getItem() : DMItems.DALEK_LASER.get());
                        AbstractDalekLaser abstractarrow = arrowitem.createArrow(pLevel, itemstack, player);
                        ...
                        if (flag1 || player.getAbilities().instabuild && (itemstack.is(Items.SPECTRAL_ARROW) || itemstack.is(Items.TIPPED_ARROW))) {
                            abstractarrow.pickup = AbstractDalekLaser.Pickup.CREATIVE_ONLY;
                        }

                        pLevel.addFreshEntity(abstractarrow);
                    }

                    pLevel.playSound((Player)null, player.getX(), player.getY(), player.getZ(), DMSound.DALEK_GUN.get(), SoundSource.PLAYERS, 1.0F, 1.0F / (pLevel.getRandom().nextFloat() * 0.4F + 1.2F) + f * 0.5F);
    ...
    public AbstractDalekLaser customArrow(AbstractDalekLaser arrow) {
        return arrow;
    }
