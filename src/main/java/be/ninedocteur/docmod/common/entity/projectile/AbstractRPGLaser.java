...
public abstract class AbstractRPGLaser extends Projectile {
    ...
    public AbstractRPGLaser.Pickup pickup;
    ...
    protected AbstractRPGLaser(EntityType<? extends AbstractRPGLaser> p_36721_, Level p_36722_) {
        super(p_36721_, p_36722_);
        this.pickup = AbstractRPGLaser.Pickup.DISALLOWED;
        this.baseDamage = 10.0D;
    }

    protected AbstractRPGLaser(EntityType<? extends AbstractRPGLaser> p_36711_, double p_36712_, double p_36713_, double p_36714_, Level p_36715_) {
        this(p_36711_, p_36715_);
        this.setPos(p_36712_, p_36713_, p_36714_);
    }

    protected AbstractRPGLaser(EntityType<? extends AbstractRPGLaser> p_36717_, LivingEntity p_36718_, Level p_36719_) {
        ...
        if (p_36718_ instanceof Player) {
            this.pickup = AbstractRPGLaser.Pickup.ALLOWED;
        }
    ...
    public void tick() {
        ...
        if (this.inGround && !flag) {
           ...
           // this.setNoPhysics(true);
            ...
            this.level.addParticle(ParticleTypes.CLOUD, this.getX() + d5 *4.0D, this.getY() + d6 * 4.0D, this.getZ() + d1 * 4.0D, -d5, -d6 + 0.2D, -d1);
            this.level.addParticle(ParticleTypes.CLOUD, this.getX() + d5 *4.0D, this.getY() + d6 * 4.0D, this.getZ() + d1 * 4.0D, -d5, -d6 + 0.2D, -d1);
            this.level.addParticle(ParticleTypes.CLOUD, this.getX() + d5 *4.0D, this.getY() + d6 * 4.0D, this.getZ() + d1 * 4.0D, -d5, -d6 + 0.2D, -d1);
            this.level.addParticle(ParticleTypes.CLOUD, this.getX() + d5 *4.0D, this.getY() + d6 * 4.0D, this.getZ() + d1 * 4.0D, -d5, -d6 + 0.2D, -d1);
            this.level.addParticle(ParticleTypes.CLOUD, this.getX() + d5 *4.0D, this.getY() + d6 * 4.0D, this.getZ() + d1 * 4.0D, -d5, -d6 + 0.2D, -d1);
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        if (entity1 == null) {
            damagesource = DMDamageSource.rpgLaser(this, this);
        } else {
            damagesource = DMDamageSource.rpgLaser(this, entity1);
        ...
        if (entity.hurt(damagesource, (float)i)) {
            ...
            if (!this.level.isClientSide && this.getDeltaMovement().lengthSqr() < 1.0E-7D) {
                if (this.pickup == AbstractRPGLaser.Pickup.ALLOWED) {
    ...
    public void readAdditionalSaveData(CompoundTag pCompound) {
        ...
        this.pickup = AbstractRPGLaser.Pickup.byOrdinal(pCompound.getByte("pickup"));
    ...
    public void setOwner(@Nullable Entity pEntity) {
        ...
        if (pEntity instanceof Player) {
            this.pickup = ((Player)pEntity).getAbilities().instabuild ? AbstractRPGLaser.Pickup.CREATIVE_ONLY : AbstractRPGLaser.Pickup.ALLOWED;
        }
    ...
    static {
        ID_FLAGS = SynchedEntityData.defineId(AbstractRPGLaser.class, EntityDataSerializers.BYTE);
        PIERCE_LEVEL = SynchedEntityData.defineId(AbstractRPGLaser.class, EntityDataSerializers.BYTE);
    }
