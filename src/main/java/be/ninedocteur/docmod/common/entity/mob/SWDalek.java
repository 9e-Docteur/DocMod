...
public class SWDalek extends Monster implements RangedAttackMob {

    private final RangedBowAttackGoal bowGoal = new RangedBowAttackGoal(this, 0.25D, 80, 20.0F);
    private final MeleeAttackGoal meleeGoal = new MeleeAttackGoal(this, 0.2D, false);

    public SWDalek(EntityType<? extends Monster> p_27403_, Level p_27404_) {
    ...
    protected void addBehaviourGoals() {
        ...
        this.targetSelector.addGoal(9, new RangedAttackGoal(this, 0.25, 80, 20));
    }

    public static AttributeSupplier.Builder createAttributes() {
        return Mob.createMobAttributes().add(Attributes.MAX_HEALTH, 50.0D).add(Attributes.FOLLOW_RANGE, 35.0D).add(Attributes.ATTACK_DAMAGE, 3.0D).add(Attributes.ARMOR, 2.0D).add(Attributes.MOVEMENT_SPEED, 1F);
    }
    ...
    public void performRangedAttack(LivingEntity pTarget, float pDistanceFactor) {
        ItemStack itemstack = this.getCyberLaserProjectile(this.getItemInHand(ProjectileUtil.getWeaponHoldingHand(this, (item) -> {
            return item instanceof RPGGun;
        })));
        AbstractRPGLaser abstractarrow = this.getArrow(itemstack, pDistanceFactor);
        if (this.getMainHandItem().getItem() instanceof RPGGun) {
            abstractarrow = ((RPGGun)this.getMainHandItem().getItem()).customArrow(abstractarrow);
        }
        ...
        this.playSound(DMSound.SPECIAL_WEAPON.get(), 1.0F, 1.0F / (this.getRandom().nextFloat() * 0.4F + 0.8F));
        this.level.addFreshEntity(abstractarrow);
    }

    protected AbstractRPGLaser getArrow(ItemStack pArrowStack, float pDistanceFactor) {
        return this.getMobArrow(this, pArrowStack, pDistanceFactor);
    }

    public ItemStack getCyberLaserProjectile(ItemStack pShootable) {
        if (pShootable.getItem() instanceof RPGGunItem) {
            Predicate<ItemStack> $$1 = ((RPGGunItem)pShootable.getItem()).getSupportedHeldProjectiles();
            ItemStack $$2 = RPGGunItem.getHeldProjectile(this, $$1);
            return $$2.isEmpty() ? new ItemStack(DMItems.RPG_LASER.get()) : $$2;
        } else {
            return ItemStack.EMPTY;
        }
    }

    public static AbstractRPGLaser getMobArrow(LivingEntity pShooter, ItemStack pArrowStack, float pDistanceFactor) {
        RPGItem arrowitem = (RPGItem) (pArrowStack.getItem() instanceof RPGItem ? pArrowStack.getItem() : DMItems.RPG_LASER.get());
        AbstractRPGLaser abstractarrow = arrowitem.createArrow(pShooter.level, pArrowStack, pShooter);
