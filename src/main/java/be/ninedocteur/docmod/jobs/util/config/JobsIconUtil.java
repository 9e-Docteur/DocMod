        ...
        if(folder.isDirectory()){
            for(String job : ServerJobsData.JOBS_LEVELS.getJobs()){
                ...
                try {
                    ...
                    DocMod.LOGGER.info("Error loading icons : " + file.getName() + " does not exist !", true);
                } catch (IOException e) {
                    DocMod.LOGGER.info("Error loading icon file <" + file.getName() + "> !", true);
                }
            }
        }
    }
    ...
    public static Map<String, DynamicTexture> convertToClient(Map<String, byte[]> map){
        ...
        for(Map.Entry<String, byte[]> e : map.entrySet()){
            try {
                ...
                DocMod.LOGGER.info("Error receiving icon from network ! (job: " + e.getKey() + ")", true);
            }
            catch (IllegalStateException exception) {
                DocMod.LOGGER.info("Icon for job <" + e.getKey() + "> must be 64x64 !", true);
            }
        }
        return newMap;
    }
